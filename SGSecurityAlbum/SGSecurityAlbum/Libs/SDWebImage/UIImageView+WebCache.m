...
- (void)sd_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletionBlock)completedBlock {
    ...
    if (url) {
        ...
        // SDWebImageOperation协议规定了一个可取消的operation
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadImageWithURL:url options:options progress:progressBlock completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            ...
            dispatch_main_sync_safe(^{
                ...
                if (image && (options & SDWebImageAvoidAutoSetImage) && completedBlock)
                {
                    ...
                    return;
                } else if (image) { // 图片完成，通过设置图片的方式直接显示，并且要求重新布局
                    wself.image = image;
                    [wself setNeedsLayout];
                } else {
                    if ((options & SDWebImageDelayPlaceholder)) { // 图片未完成，显示占位图片
                        wself.image = placeholder;
                        [wself setNeedsLayout];
                    }
                }
                if (completedBlock && finished) { // 只要图片完成，就回调完成
                    completedBlock(image, error, cacheType, url);
                }
            });
        }];
        // 将这个异步任务保存到与当前UIImageView绑定的字典中，会先去检查是否存在一个任务，如果存在则取消
